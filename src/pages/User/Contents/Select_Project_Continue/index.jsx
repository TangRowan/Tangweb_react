...
class Crequest extends Component {
  ...
  componentDidMount(){
    axios.get(this.props.url+'/IEEEbackend/searchdata.php?!='+('6|'+this.props.Email)).then(
      response => {
  ...
  render() {
    ...
    const columns = [
      ...
      
      {
        ...
        render: (text, record) => {
          ...
          return(//{record.Name}
            <Space size="small">
              <Action code={record.ProjectID} name={record.ProjectTitle} id={record.key}/>
            </Space>
          )
         }
    ...
    return (
      ...
      <Card className='ncenter' title="Select porject to Continue Proposal" bordered={false}>
