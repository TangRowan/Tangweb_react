...
import { Affix,Form, Input, Button,  Card, Space,Modal,notification,Spin,Select,Rate,Tooltip,Col,Row} from 'antd'
import './index.css'
...
export default function GoEva() {
  ...
  const onFinish = (values) => {
    ...
    //console.log('Success:', values);
        ...
        //console.log('Success:', values);
        axios.get(userstate.Backurl.url+'/IEEEbackend/rank.php?!=0|'+((params.IID)+'|'+values.ToFund+'|'+values.Reason+'|'+values.Rank+'|'+userstate.userinf.Email)).then(
            response => {
  ...
  useEffect(() => {
    axios.get(userstate.Backurl.url+'/IEEEbackend/browse.php?!=1|'+params.IID).then(
      response => {
...
<p/>
     ...
     <p/>
      ...
      <Tooltip title="Move the mouse over the star and click. The score range is 0 ~ 5 points. Double click star give 0 points.">
  
      
      <Form.Item label="Rank:" name="Rank" wrapperCol={{ span: 3 }}rules={[{ required: true,message: 'Choise Rank', }]} >
      <Rate   allowHalf />
      </Form.Item>
      </Tooltip>
